...
from Projekt.coordconv import CoordConv,AddCoords
...
class AddSine(AddCoords):
    ...
    def generate_xy(self, input_tensor):
        ...
        yy_channel = torch.linspace(0.,1.,y_dim).repeat(1, x_dim, 1).to(self.phase.device)

        xx_channel = xx_channel.float()*self.alpha
        yy_channel = yy_channel.float()*self.beta

        channel = xx_channel.repeat(batch_size,1,1,1).transpose(2,3) + yy_channel.repeat(batch_size,1,1,1).transpose(2,3)
    ...
    def __repr__(self):
        return self.__class__.__name__ + ' (' 'alpha=' + str(self.alpha.item()) + ' beta=' +str(self.beta.item()) + 'phase=' + str(self.phase.item()) +') '
