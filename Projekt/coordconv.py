...
class AddCoords(nn.Module):
    ...
    def generate_xy(self, input_tensor):
        batch_size, channel, x_dim, y_dim = input_tensor.size()

        xx_channel = torch.arrange(x_dim).repeat(1, y_dim, 1)
        yy_channel = torch.arrange(y_dim).repeat(1, x_dim, 1).transpose(1, 2)
